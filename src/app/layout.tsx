...
async function getSiteSettings() {
  try {
    ...
    const [n, d, og, lo, ts] = await Promise.all([
    ...
    return {
      ...
      titleSuffix: (ts as any)?.value ?? "กล่องสุ่มสินค้าพรีเมียม",
    };
  } catch {
    return {
      siteName: "LuxusX",
      ...
      titleSuffix: "กล่องสุ่มสินค้าพรีเมียม",
    };
  }
}

export async function generateMetadata(): Promise<Metadata> {
  const { siteName, siteDesc, ogImage, siteLogo, titleSuffix } = await getSiteSettings();
  const title = titleSuffix ? `${siteName} - ${titleSuffix}` : siteName;
  return {
      ...
      <body suppressHydrationWarning className={`${notoSansThai.className} bg-[#F8F8F8] text-gray-900 flex h-screen overflow-hidden`}>
        <noscript>
          ...
          <img
            ...
            width="1"
            style={{ display: "none" }}
            src="https://www.facebook.com/tr?id=2508621746202378&ev=PageView&noscript=1"
            alt=""
          />
        </noscript>
        <Providers>
        ...
        <Script
          ...
          strategy="afterInteractive"
          dangerouslySetInnerHTML={{
            __html: `!function(f,b,e,v,n,t,s){if(f.fbq)return;n=f.fbq=function(){n.callMethod?n.callMethod.apply(n,arguments):n.queue.push(arguments)};if(!f._fbq)f._fbq=n;n.push=n;n.loaded=!0;n.version='2.0';n.queue=[];t=b.createElement(e);t.async=!0;t.src=v;s=b.getElementsByTagName(e)[0];s.parentNode.insertBefore(t,s)}(window,document,'script','https://connect.facebook.net/en_US/fbevents.js');fbq('init', '2508621746202378');fbq('track', 'PageView');`,
          }}
        />
      </body>
    </html>
