...
import { Plus, Edit2, Trash2, X, Package, Box as BoxIcon, ShoppingBag } from "lucide-react";
...
const emptyForm = {
  ...
  itemId: "",
  shopListingId: "",
  honeyCost: 100, stock: 0, isActive: true,
};

export default function HoneycombRewardsPage() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const [rRes, bRes, iRes, sRes, settRes] = await Promise.all([
        fetch("/api/admin/honeycomb-rewards"),
      ...
      if (settRes.ok) {
        ...
        if (s.honeycomb_coin_name) setCoinName(s.honeycomb_coin_name);
      }
  ...
  const openEdit = (r: HoneycombReward) => {
    setModalMode("edit");
    setEditId(r._id);
    setForm({
      name: r.name,
  ...
  const handleSave = async () => {
    if (!form.name || !form.honeyCost) { alert("กรุณากรอกชื่อและค่าเหรียญ"); return; }
    ...
    setSaving(true);
    try {
      const url = modalMode === "add" ? "/api/admin/honeycomb-rewards" : `/api/admin/honeycomb-rewards/${editId}`;
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`/api/admin/honeycomb-rewards/${id}`, { method: "DELETE" });
  ...
  const getTypeLabel = (r: HoneycombReward) => {
  ...
  const getThumb = (r: HoneycombReward) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">แลก{coinName}</h1>
          <p className="text-sm text-slate-500 mt-1">จัดการรายการแลกรางวัลด้วย{coinName}</p>
        </div>
        <button onClick={openAdd} className="flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-lg font-bold text-sm transition-colors shadow-sm shrink-0">
          <Plus size={16} />
          เพิ่มรางวัล
        </button>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50/50">
              ...
              <th className="py-4 px-6 text-[11px] font-bold text-slate-400 uppercase tracking-wider text-right">ค่า{coinName}</th>
                ...
                <tr key={r._id} className="hover:bg-slate-50/80 transition-colors group">
                  ...
                  <td className="py-4 px-6 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <span className="font-black text-amber-700">{r.honeyCost.toLocaleString()}</span>
                    </div>
                  </td>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="p-6 flex flex-col gap-4 overflow-y-auto">
              <div>
                ...
                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-sm outline-none focus:border-amber-500 font-medium text-slate-800"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <select value={form.boxId} onChange={(e) => setForm({ ...form, boxId: e.target.value })}
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-amber-500 font-medium text-slate-800">
                  ...
                  <div>
                    ...
                    <input type="number" min="1" value={form.boxOpenTimes}
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-amber-500 font-black text-slate-800" />
                  </div>
                </div>
              )}
                  ...
                  <select value={form.itemId} onChange={(e) => setForm({ ...form, itemId: e.target.value })}
                    ...
                    {items.map((i) => (
                      <option key={i._id} value={i._id}>
                        {i.type === "coin_reward" ? `🍯 ${i.name} (+${i.coinRewardAmount})` : i.name}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-600 mb-1.5">ค่า{coinName} <span className="text-red-500">*</span></label>
                  <input type="number" min="1" value={form.honeyCost}
                    onChange={(e) => setForm({ ...form, honeyCost: parseInt(e.target.value) || 0 })}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-amber-500 font-black text-amber-700" />
                </div>
                {form.type !== "shop" && (
                  <div>
            ...
            <div className="p-6 border-t border-slate-100 flex items-center justify-end gap-3 shrink-0">
              ...
              <button onClick={handleSave} disabled={saving}
                className="px-5 py-2.5 rounded-lg text-sm font-bold bg-amber-500 hover:bg-amber-600 disabled:bg-amber-300 text-white transition-colors shadow-sm">
