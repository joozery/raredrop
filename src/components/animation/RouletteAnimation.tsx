...
export function RouletteAnimation({ boxItems, resultItem, onEnded }: RouletteAnimationProps) {
    ...
    <>
      ...
      <div style={{ position: "relative", width: dims.rootWidth, maxWidth: "1280px", margin: dims.rootMargin }}>
        ...
        <div style={{
          ...
          <div style={{
            position: "absolute", inset: 10, borderRadius: dims.rimRadius, pointerEvents: "none",
            border: "1.5px solid transparent",
            background: "linear-gradient(165deg,#63666f,#15161a 30%,#000 55%,#4a4c55 100%) border-box",
          ...
          {["top", "bottom"].map((side) => (
            <div key={side} style={{
              ...
              width: "34%", height: 10,
              ...(side === "top" ? { top: 6 } : { bottom: 6 }),
              background: "linear-gradient(180deg,#26272d,#0b0c0f)",
              borderRadius: 6,
              boxShadow: "0 0 0 1px #000, 0 1px 0 rgba(255,255,255,.05) inset",
            }} />
          ))}

          {/* red corner light strips */}
          {[
            { cls: "", style: { top: -2, left: "12%" } },
            { cls: "rd-strip-tr", style: { top: -2, right: "12%" } },
            { cls: "rd-strip-bl", style: { bottom: -2, left: "12%" } },
            { cls: "rd-strip-br", style: { bottom: -2, right: "12%" } },
          ].map((s, i) => (
            <div key={i} className={`rd-strip ${s.cls}`} style={{
              position: "absolute", height: 5, width: 120, borderRadius: 4,
              background: "#ff2d3f", filter: "blur(1px)",
              boxShadow: "0 0 18px 4px rgba(255,45,63,.7)",
              ...s.style,
            }} />
          ))}

          {/* side vertical glows */}
          {[{ cls: "", left: 2 }, { cls: "rd-sideglow-r", right: 2 }].map((s, i) => (
            <div key={i} className={`rd-strip ${s.cls}`} style={{
              position: "absolute", top: "26%", bottom: "26%", width: 8, borderRadius: 8,
              background: "linear-gradient(180deg,transparent,#ff2d3f,transparent)",
              filter: "blur(2px)",
              boxShadow: "0 0 22px 5px rgba(255,45,63,.5)",
              ...(i === 0 ? { left: 2 } : { right: 2 }),
            }} />
          ))}

          {/* ‹ arrow */}
          {dims.showArrows && <button style={{
            position: "absolute", top: "50%", transform: "translateY(-50%)", left: 20, zIndex: 8,
            background: "none", border: 0, cursor: "pointer", padding: 8,
            color: "#ff2d3f", fontSize: 34, lineHeight: 1, fontWeight: 800,
            textShadow: "0 0 10px rgba(255,45,63,.9), 0 0 24px rgba(255,45,63,.5)",
            fontFamily: "inherit",
          }}>‹</button>}

          {/* › arrow */}
          {dims.showArrows && <button style={{
            position: "absolute", top: "50%", transform: "translateY(-50%)", right: 20, zIndex: 8,
            background: "none", border: 0, cursor: "pointer", padding: 8,
            color: "#ff2d3f", fontSize: 34, lineHeight: 1, fontWeight: 800,
            textShadow: "0 0 10px rgba(255,45,63,.9), 0 0 24px rgba(255,45,63,.5)",
            fontFamily: "inherit",
          ...
          <div ref={windowRef} style={{
            ...
            borderRadius: dims.winRadius, padding: "22px 0",
          }}>
            {/* center red line */}
            <div style={{
            ...
            {/* ── Card track ── */}
            {/* style ของ track ต้องนิ่ง (ไม่ผูก state) — ถ้า React re-render แล้วเขียน style ทับ
                transition ที่สั่งหมุนด้วย JS จะหลุดกลางคัน (รางกระโดดไปจุดจบทันที) */}
              ...
              {items.map((item, idx) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderColor: revealed
                        ? "#ff2d3f"
                        : (!rarity?.backgroundImage ? (rc ? `${rc}60` : "#e2e8f0") : undefined),
                      borderWidth: revealed ? 2.5 : (!rarity?.backgroundImage ? 2 : 1),
                      boxShadow: revealed
                        ? `0 0 0 2px #ff2d3f, 0 0 28px rgba(255,45,63,.9), 0 0 60px rgba(255,45,63,.4), inset 0 0 20px rgba(255,45,63,.3)`
                        : undefined,
                  ...
                  >
                    {/* Winner ring */}
                    {revealed && (
                      <div className="absolute inset-0 rounded-[18px] z-40 pointer-events-none animate-pulse"
                        style={{ boxShadow: "inset 0 0 0 2px rgba(255,255,255,0.6), 0 0 20px rgba(255,255,255,0.3)" }} />
                    )}

                    {/* Rarity background image */}
                    {rarity?.backgroundImage && (
                      <div className="absolute inset-0 z-0 bg-[#0a0a14]">
                        <img src={rarity.backgroundImage} alt="bg" className="w-full h-full object-cover opacity-80" />
                    ...
                    {/* Rarity badge */}
                    <div
                      className="absolute top-2 left-2 text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-wider z-20 text-white shadow-sm flex items-center gap-1"
                      style={{ backgroundColor: revealed ? "#ff2d3f" : (rc || "#64748b") }}
                    >
                      <Sparkles size={9} className={revealed ? "animate-pulse" : ""} />
                      <span>{rarity?.name || "N/A"}</span>
                    </div>

                    {/* Item image box */}
                    <div
                      className="mx-auto flex items-center justify-center rounded-[14px] relative z-10 transition-all duration-300 overflow-hidden"
                      style={{
                        width: dims.img, height: dims.img, marginTop: dims.imgMt,
                        boxShadow: rarity?.backgroundImage ? `0 0 15px ${rc}60, inset 0 0 10px ${rc}30` : "none",
                        border: rarity?.backgroundImage ? `1.5px solid ${rc}` : "2px solid transparent",
                        backgroundColor: !rarity?.backgroundImage ? "#f8fafc" : undefined,
                        ...(revealed ? { boxShadow: `0 0 20px ${rc}, inset 0 0 10px ${rc}`, border: `2px solid ${rc}` } : {}),
                      }}
                    >
                      <div className="relative w-full h-full flex items-center justify-center p-2">
                        {item?.image ? (
                          <img
                            src={item.image}
                            alt={item.name}
                            className={`w-full h-full object-contain transition-transform duration-500 ${revealed ? "scale-125" : ""} ${!rarity?.backgroundImage ? "mix-blend-multiply drop-shadow-none" : "drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]"}`}
                          />
                        ) : (
                    ...
                    <div className="mt-3 mb-2 flex flex-col items-center text-center relative z-10 px-3">
                      {rarity?.backgroundImage && (
                        <div className="flex items-center w-full justify-center gap-2 mb-1 opacity-70">
                          <div className="h-[1px] w-6 bg-gradient-to-r from-transparent to-white/60" />
                          <Sparkles size={9} className="text-white" />
                          <div className="h-[1px] w-6 bg-gradient-to-l from-transparent to-white/60" />
                        </div>
                      )}
                      <p
                        className={`font-bold text-[13px] line-clamp-1 w-full transition-colors ${rarity?.backgroundImage ? "text-white" : (revealed ? "text-red-500" : "text-gray-800")}`}
                        style={revealed ? { textShadow: "0 0 12px rgba(255,45,63,.75)" } : undefined}
                      >
                        {item?.name}
                      </p>
                      <p className={`text-[10px] leading-tight mt-1 font-medium ${rarity?.backgroundImage ? "text-white/50" : "text-gray-400"}`}>
                        ไอเทมระดับ {rarity?.name || "Common"}
                      </p>
                    </div>
                  </div>
                );
